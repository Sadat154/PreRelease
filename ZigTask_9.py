...
class Puzzle():
    def __init__(self, *args):
        if len(args) == 1:
            ...
            for i in range(0, 5):  # 5 D's
                x = random.randrange(0, self.__GridSize * self.__GridSize)
                DLocations.append(x)
                self.__Grid[x] = DoubleCell()
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                AmountToAddToScore = self.CheckforMatchWithPattern(Row, Column, Symbol)
                print(AmountToAddToScore)
    ...
    def CheckforMatchWithPattern(self, Row, Column, InputSymbol):
            ...
            for StartColumn in range(Column - 2, Column + 1):
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        CurrentSymbol = InputSymbol#self.__GetCell(Row, Column).GetSymbol()

                        if P.MatchesPattern(PatternString, InputSymbol):

                            ########CHECK DOUBLE###########
                            Score = 10
                            print("TEST1")
                            if (self.__GetCell(StartRow, StartColumn).IsDouble()) or (self.__GetCell(StartRow, StartColumn + 1).IsDouble()) or (self.__GetCell(StartRow, StartColumn + 2).IsDouble()) or (self.__GetCell(StartRow - 1, StartColumn + 2).IsDouble()) or (self.__GetCell(StartRow - 2, StartColumn + 2).IsDouble()) or (self.__GetCell(StartRow - 2, StartColumn + 1).IsDouble()) or (self.__GetCell(StartRow - 2, StartColumn).IsDouble()) or (self.__GetCell(StartRow - 1, StartColumn).IsDouble()) or (self.__GetCell(StartRow - 1, StartColumn + 1).IsDouble()):
                                print("TEST2")
                                Score = 20
                            print("TEST3")
                            ########CHECK DOUBLE###########
...
class Pattern():
    ...
    def MatchesPattern(self, PatternString, SymbolPlaced):
        if SymbolPlaced != self.__Symbol and SymbolPlaced != 'D':
...
class DoubleCell(Cell):
    def __init__(self):
        super(DoubleCell, self).__init__()
        self._Symbol = "D"

...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if x == 1: #25% trigger swamp event
                #No Turns warning

                print(f"WARNING! A swamp event has been triggered to occur in {Warning} rounds!")
                Warning-=1
                if Warning == -1:
                    self.SwampThisCell()
                    x = 69
    ...
    def SwampThisCell(self):
        NumOfSwamps = random.randint(1,4)
        PositionsToModify = []
        #Random Grid positions

        while len(PositionsToModify) != NumOfSwamps:
            x = random.randrange(0, self.__GridSize**2)
            print(x)
            print(self.__Grid[x].GetSymbol())
            if self.__Grid[x].GetSymbol() == "-":
                PositionsToModify.append(x)
        #Modifications
        for i in PositionsToModify:
            self.__Grid[i] = BlockedCell()
            self.__Grid[i].UpdateCell()
            self.__Grid[i].GetSymbol()

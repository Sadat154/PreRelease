...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                if CurrentCell.CheckPartOfPattern():
                    self.RemoveSymbol(Row,Column, CurrentCell.GetSymbol())

                CurrentCell.ChangeSymbolInCell(Symbol)
                AmountToAddToScore = self.CheckforMatchWithPattern(Row, Column, "Normal")
    ...
    def RemoveSymbol(self, Row, Column, Symbol):
        CurrentCell = self.__GetCell(Row,Column)
        if (not CurrentCell.CheckSymbolAllowed(Symbol)):


            AmountToAddToScore = self.CheckforMatchWithPattern(Row,Column,"NotNormal")
            self.__Grid[(self.__GridSize - Row) * self.__GridSize + Column - 1].ChangeSymbolInCell("")
            if AmountToAddToScore < 0:
                self.__Score += AmountToAddToScore
    ...
    def CheckforMatchWithPattern(self, Row, Column, Flag):
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        if P.MatchesPattern(PatternString, CurrentSymbol):
                            if Flag == "Normal":
                                ...
                                self.__GetCell(StartRow, StartColumn).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow, StartColumn).UnsetSetCellAsPartOfAPattern()

                                self.__GetCell(StartRow, StartColumn + 1).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow, StartColumn + 1).UnsetSetCellAsPartOfAPattern()

                                self.__GetCell(StartRow, StartColumn + 2).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow, StartColumn + 2).UnsetSetCellAsPartOfAPattern()

                                self.__GetCell(StartRow - 1, StartColumn + 2).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow - 1, StartColumn + 2).UnsetSetCellAsPartOfAPattern()

                                self.__GetCell(StartRow - 2, StartColumn + 2).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow - 2, StartColumn + 2).UnsetSetCellAsPartOfAPattern()

                                self.__GetCell(StartRow - 2, StartColumn + 1).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow - 2, StartColumn + 1).UnsetSetCellAsPartOfAPattern()

                                self.__GetCell(StartRow - 2, StartColumn).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow - 2, StartColumn).UnsetSetCellAsPartOfAPattern()

                                self.__GetCell(StartRow - 1, StartColumn).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow - 1, StartColumn).UnsetSetCellAsPartOfAPattern()

                                self.__GetCell(StartRow - 1, StartColumn + 1).RemoveSymbolFromNotAllowed(CurrentSymbol)
                                self.__GetCell(StartRow - 1, StartColumn + 1).UnsetSetCellAsPartOfAPattern()
...
class Cell():
    ...
    def SetCellAsPartOfAPattern(self):
        self._PartOfAPattern = True

    def UnsetSetCellAsPartOfAPattern(self):
        self._PartOfAPattern = False


    def CheckPartOfPattern(self):
        return self._PartOfAPattern
    ...
    def AddToNotAllowedSymbols(self, SymbolToAdd):
        ...
        self.SetCellAsPartOfAPattern()

    def RemoveSymbolFromNotAllowed(self, SymbolToRemove):

...
class Puzzle():
    ...
    def TestForError1(self):
        for Symbol in self.__AllowedSymbols:
            for Row in range(self.__GridSize, 2, -1):
                for Column in range(1, self.__GridSize):
                    if len(self.__Grid[(self.__GridSize - Row) * self.__GridSize + Column - 1].GetSymbolsNotAllowed()) > 0 and self.__Grid[(self.__GridSize - Row) * self.__GridSize + Column - 1].GetSymbol() != "@" and self.__Score == 0:
                        print("There are cells in which error 1 is demonstrated")

    def TestForError2(self):
        CellsWithSymbolsNotAllowed = 0
        for i in range(0, self.__GridSize**2):
            if len(self.__Grid[i].GetSymbolsNotAllowed()) > 0:
                CellsWithSymbolsNotAllowed += 1

        if CellsWithSymbolsNotAllowed < 9 * ((self.__Score)/10):
            print("There is an error with symbols not allowed where some cells are missing these symbols not allowed!")

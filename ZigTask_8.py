...
class Puzzle():
    ...
    def ReShuffleBlockedCells(self):
        ...
        for cell in self.__Grid:
            ...
            if cell.GetSymbol() == '@':
                BlockedCellLocations.append(CounterIndex)
            elif cell.GetSymbol() in self.__AllowedSymbols:
                SymbolLocations.append(CounterIndex)


        NewLocations = []

        while len(NewLocations) != len(BlockedCellLocations):
            x = random.randrange(0,self.__GridSize**2)
            if x not in SymbolLocations and x not in BlockedCellLocations:
                NewLocations.append(x)

        for i in NewLocations:
            self.__Grid[i].ChangeSymbolInCell("@")
            self.__Grid[i].AddToNotAllowedSymbols("@")

        for i in BlockedCellLocations:
            self.__Grid[i].ChangeSymbolInCell("")
            self.__Grid[i].RemoveNotAllowedSymbols("@")
...
class Cell():
    ...
    def RemoveNotAllowedSymbols(self, SymbolToRemove):
        if "@" in self.__SymbolsNotAllowed:
            self.__SymbolsNotAllowed.remove('@')

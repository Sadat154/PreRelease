...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                AmountToAddToScore, StartingPos = self.CheckforMatchWithPattern(Row, Column)
                if AmountToAddToScore > 0:
                    self.__Score += AmountToAddToScore
                    self.__Score += self.CheckForOverlap(Symbol, Row, Column)
                    #Add code here that checks
    ...
    def CheckForOverlap(self, CurrentSymbol, StartingRow, StartingColumn):
        Score = 0
        for row in range(StartingRow, (StartingRow - 3), -1):
            for column in range(StartingColumn, (StartingColumn + 3)):
                try:
                    CurrentCell = self.__GetCell(row, column)
                    NotAllowedSymbols = CurrentCell.GetNotAllowedSymbols()
                    for Symbol in NotAllowedSymbols:
                ...
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        if P.MatchesPattern(PatternString, CurrentSymbol):
                            ...
                            return 10, [StartRow, StartColumn]
                except:
                    pass
        return 0
